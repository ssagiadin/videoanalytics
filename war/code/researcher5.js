...
function postEdit(response) {
	if (response == "OK") {
		...
		$("input:eq(4)", $row).val($("#pheight").val());
		$("input:eq(5)", $row).val($("#pwidth").val());
...
/* This function is executed when the user presses the Get Code button and
 * prints out the code that has to inserted in a page to enable videoanalysis.
 */
function getcode() {
	$row = $(this).closest("li"); // List item that contains the experiment data
...
//	$("#urlvideoid").html("http://localhost:8888/?videoId=" + expid);
	$("#codearea").text('<div data-videoanalysis="true" data-expid="' + expid + '" ></div>');
	$("#codeinfo").popup('open');
}
...
function handleQueryResponse(response) {
	...
	var transaction, range, begin, end, sign;
	var data = response.getDataTable(); //Get DataTable from Fusion Table response
	var length = data.getNumberOfRows();
	var charttable = new google.visualization.DataTable(); // Create table to hold timeline values
	charttable.addColumn('number', 'Second'); // Add timeline column
	charttable.addColumn('number', 'Count')
	for (var i = 0; i < length; i++) { // Process all records
		transaction = data.getValue(i, 1); // Record transaction type
		if (transaction == 1 || transaction == 2 || transaction == 5) { // Process Backward, Forward and Seekbar
			range = data.getValue(i, 0) // Transaction time begin - end 
			begin = range >>> 16; // Begin time at bits 16-31
			end = range & rangemask; // End time at bits 0-15
			if (begin > end) { // Swap begin - end
				var t = begin;
				begin = end;
				end = t;
				sign = 1; // On backward increase timeline values
			} else
				sign = -1; // On forward decrease timeline values
			for (var r = charttable.getNumberOfRows(); r <= end; r++) // Add timeline rows if they do not exist
				charttable.addRow([r, 0]);
			var expval = sign * (data.getValue(i, 2) * 2) // Double the experiment transaction count
			for (var j = begin; j <= end; j++) // For all timeline values in range add this value
				charttable.setValue(j, 1, charttable.getValue(j, 1) + expval);
		}
	}
	var chart = new google.visualization.LineChart(document.getElementById('visualization'));
	chart.draw(charttable, {width: 500, height: 525, strictFirstColumnType: true});
...
//	  view.setColumns([0, {calc:summary, type:'number', label:'SocialSkip'}]);
	var chart = new google.visualization.LineChart(document.getElementById('visualization'));
	chart.draw(view, {width: 500, height: 525, strictFirstColumnType: true});
	$.mobile.changePage($("#datainfo"));
}
